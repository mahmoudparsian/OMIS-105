"""
plot_helpers.py
===============
Simple plotting functions for OMIS 105 — Weeks 1–3 notebook.

Students: You do NOT need to read or modify this file.
          It is imported by the notebook to keep plotting
          code out of your way so you can focus on SQL.

Functions
---------
  plot_bar(df, x, y, title)       — vertical bar chart
  plot_hbar(df, x, y, title)      — horizontal bar chart
  plot_pie(df, labels, values, title) — pie / donut chart
  plot_grouped_bar(df, x, y_cols, title) — side-by-side bars
"""

import matplotlib.pyplot as plt
import matplotlib.ticker as mticker

# ── Colour palette ───────────────────────────────────────────
COLORS = ["#4C72B0", "#55A868", "#C44E52", "#8172B2", "#E58606",
          "#937860", "#DA8BC3", "#8C8C8C", "#CCB974", "#64B5CD"]


def _dollar_fmt(val, _=None):
    return f"${val:,.0f}"


def _apply_style(ax, title, grid_axis="y"):
    """Common styling for all charts."""
    ax.set_title(title, fontsize=14, fontweight="bold",
                 color="#2c3e50", pad=12)
    ax.spines[["top", "right"]].set_visible(False)
    ax.grid(axis=grid_axis, linestyle="--", alpha=0.4)


# ── Vertical bar chart ───────────────────────────────────────
def plot_bar(df, x, y, title, ylabel="", dollar=False,
             figsize=(8, 4), color=None):
    fig, ax = plt.subplots(figsize=figsize)
    colors = color if color else COLORS[:len(df)]
    bars = ax.bar(df[x].astype(str), df[y], color=colors,
                  edgecolor="white", linewidth=0.8)

    fmt = _dollar_fmt if dollar else (lambda v, _=None: f"{v:,.0f}")
    ax.bar_label(bars, fmt=fmt, padding=4, fontsize=10, color="#2c3e50")

    if dollar:
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(_dollar_fmt))

    ax.set_ylabel(ylabel, fontsize=11)
    _apply_style(ax, title, grid_axis="y")
    plt.tight_layout()
    plt.show()


# ── Horizontal bar chart ─────────────────────────────────────
def plot_hbar(df, x, y, title, xlabel="", dollar=False,
              figsize=(8, 4), color=None):
    fig, ax = plt.subplots(figsize=figsize)
    colors = color if color else COLORS[:len(df)]
    bars = ax.barh(df[y].astype(str), df[x], color=colors,
                   edgecolor="white", linewidth=0.8)

    fmt = _dollar_fmt if dollar else (lambda v, _=None: f"{v:,.0f}")
    ax.bar_label(bars, fmt=fmt, padding=4, fontsize=10, color="#2c3e50")

    if dollar:
        ax.xaxis.set_major_formatter(mticker.FuncFormatter(_dollar_fmt))

    ax.set_xlabel(xlabel, fontsize=11)
    ax.invert_yaxis()
    _apply_style(ax, title, grid_axis="x")
    plt.tight_layout()
    plt.show()


# ── Pie / donut chart ────────────────────────────────────────
def plot_pie(df, labels, values, title, figsize=(6, 5)):
    colors = COLORS[:len(df)]
    fig, ax = plt.subplots(figsize=figsize)
    wedges, texts, autotexts = ax.pie(
        df[values], labels=df[labels], autopct="%1.1f%%",
        startangle=90, colors=colors, pctdistance=0.78,
        wedgeprops=dict(width=0.55, edgecolor="white", linewidth=2),
    )
    for t in autotexts:
        t.set_fontsize(11)
        t.set_fontweight("bold")
        t.set_color("white")
    ax.set_title(title, fontsize=14, fontweight="bold",
                 color="#2c3e50", pad=12)
    plt.tight_layout()
    plt.show()


# ── Grouped bar chart ────────────────────────────────────────
def plot_grouped_bar(df, x, y_cols, title, ylabel="",
                     dollar=False, figsize=(9, 5)):
    import numpy as np
    labels = df[x].astype(str).tolist()
    n = len(y_cols)
    x_pos = np.arange(len(labels))
    width = 0.7 / n

    fig, ax = plt.subplots(figsize=figsize)
    for i, col in enumerate(y_cols):
        offset = (i - n / 2 + 0.5) * width
        bars = ax.bar(x_pos + offset, df[col], width,
                      label=col, color=COLORS[i % len(COLORS)],
                      edgecolor="white", linewidth=0.8)
        fmt = _dollar_fmt if dollar else (lambda v, _=None: f"{v:,.0f}")
        ax.bar_label(bars, fmt=fmt, padding=3, fontsize=9,
                     color="#2c3e50")

    ax.set_xticks(x_pos)
    ax.set_xticklabels(labels, fontsize=11)
    ax.set_ylabel(ylabel, fontsize=11)
    if dollar:
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(_dollar_fmt))
    ax.legend(fontsize=10)
    _apply_style(ax, title, grid_axis="y")
    plt.tight_layout()
    plt.show()
