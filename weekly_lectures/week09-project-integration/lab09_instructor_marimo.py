...
def _(mo):
    ...
    **Week 9 | Grading Rubric**
    ...
    This is a grading guide, not an answer key — there's no single
    correct query set to run, since every student picks their own
    domain (see the Suggested Domains table in `lab09_student_marimo.py`
    / the starter template). Use this alongside the student's submitted
    notebook.
    """)
    return
...
def _(mo):
    ...
    1. **Copying ShopSmart exactly** — the project should be a different
       domain. Deduct 10 points for using ShopSmart or trivially
       renaming it.
    2. **No M:M relationship** — this is a key requirement. Deduct 5
       points.
    3. **Queries that don't run** — partial credit for correct logic
       with syntax errors.
    4. **"Fake" data** — random strings like "asdf" should result in
       data point deductions.
    5. **Over-scoping** — don't penalize for ambitious projects with
       minor incomplete areas; reward ambition.
    """)
    return
...
def _(mo):
    ...
    **Tables**: members, trainers, class_types, classes, rooms,
    bookings, payments (7 tables)
    ...
    **Strong queries included**:
    ...
    - Transaction: book a class (check capacity, create booking, process
      payment)
...
def _(mo):
    ...
    |------|--------|
    | 1. ER Diagram | 15 |
    | 2. Normalized Schema | 15 |
    | 3. Sample Data | 10 |
    | 4. SQL Queries | 25 |
    | 5. Transaction Demo | 10 |
    | 6. Views and Indexes | 10 |
    | 7. Presentation | 15 |
    | **Total** | **100** |
    """)
    return
