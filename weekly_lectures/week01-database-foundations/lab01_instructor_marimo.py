...
def _(mo):
    ...
    Connect to DuckDB and load `./data/products.csv` into a table.
    """)
    return
...
def _(mo):
    mo.md(r"""
    **Q6.** (3 pts) Products priced $10–$50 (inclusive), sorted by price ascending.
    """)
    return
...
def _(mo):
    mo.md(r"""
    **Q8.** (4 pts) Electronics or Sports with price > $50, sorted by price descending.
    """)
    return
...
def _(mo):
    mo.md(r"""
    **Q13.** (5 pts) Electronics category statistics: count, average price, minimum price, maximum price.
    """)
    return
...
def _(mo):
    mo.md(r"""
    **Q14.** (5 pts) Price with tax (9.25% sales tax). Show the top 10 by `price_with_tax` descending.
    """)
    return
...
def _(mo):
    mo.md(r"""
    > **Note**: Students haven't formally learned `GROUP BY` yet (that's Week 3),
    but some may figure it out. Give full credit for any working solution.
    Also accept solutions using a manual total (64).
    """)
    return
