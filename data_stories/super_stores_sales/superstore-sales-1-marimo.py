...
def _(con):
    con.execute("""
        CREATE
        OR REPLACE TABLE superstore AS
        SELECT
            *,
...
def _(con):
    ...
    df_monthly = con.execute("""
        SELECT
            strftime(order_dt, '%Y-%m') AS month,
            ROUND(SUM(sales), 2) AS total_sales,
            ROUND(SUM(profit), 2) AS total_profit,
            COUNT(*) AS order_count
        FROM superstore
...
def _(con):
    ...
    df_quarterly = con.execute("""
        ...
        ORDER BY yr, qtr) AS prev_qtr_sales, ROUND((total_sales - LAG(total_sales) OVER (
        ORDER BY yr, qtr)) / LAG(total_sales) OVER (
        ORDER BY yr, qtr) * 100, 2) AS growth_pct
        FROM quarterly
        ORDER BY yr, qtr;
    """).df()
    df_quarterly
...
def _(con):
    ...
    con.execute("""
        ...
        FROM superstore
        GROUP BY customer_id, customer_name )
        SELECT CASE WHEN frequency >= 10
        AND monetary >= 5000 THEN 'Champions' WHEN frequency >= 5
        AND monetary >= 2000 THEN 'Loyal' WHEN frequency >= 3 THEN 'Regular' ELSE 'At Risk' END AS segment, COUNT(*) AS num_customers, ROUND(AVG(monetary), 2) AS avg_revenue, ROUND(AVG(frequency), 1) AS avg_orders
        FROM customer_rfm
        GROUP BY segment
...
def _(con):
    ...
    con.execute("""
        SELECT
            CASE WHEN discount = 0 THEN 'No discount' WHEN discount <= 0.2 THEN '1-20%' WHEN discount <= 0.4 THEN '21-40%' ELSE '41%+' END AS discount_band,
            COUNT(*) AS order_count,
            ROUND(SUM(sales), 2) AS total_sales,
            ROUND(SUM(profit), 2) AS total_profit,
...
def _(con):
    ...
    con.execute("""
        SELECT
            dayname(order_dt) AS day_of_week,
...
def _(con):
    ...
    con.execute("""
        ...
        ORDER BY yr) AS prev_year, ROUND((sales - LAG(sales) OVER (PARTITION BY category
        ORDER BY yr)) / LAG(sales) OVER (PARTITION BY category
        ORDER BY yr) * 100, 2) AS yoy_growth_pct
        FROM yearly_cat
...
def _(con):
    ...
    con.execute("""
        ...
        FROM superstore
        GROUP BY product_name ), ranked AS (
        SELECT
            *,
            SUM(total_sales) OVER (
        ORDER BY total_sales DESC) AS running_total, SUM(total_sales) OVER () AS grand_total, ROW_NUMBER() OVER (
        ORDER BY total_sales DESC) AS rn, COUNT(*) OVER () AS total_products
        FROM product_sales )
        SELECT MIN(rn) FILTER (
        WHERE running_total >= grand_total * 0.8) AS products_for_80pct_sales, total_products AS total_products, ROUND(MIN(rn) FILTER (
        WHERE running_total >= grand_total * 0.8) * 100.0 / total_products, 1) AS pct_of_products
        FROM ranked
