...
def _():
    ...
    # (keeps this notebook uncluttered)
    ...
    print("✅ DuckDB connected — ready for SQL!")
    print("DuckDB version: ", duckdb.__version__)
    return con, display_result, plot_bar, plot_hbar, plot_pie
...
def _(mo):
    ...
    **Key Concepts from Slides:**
    - `CREATE TABLE` defines the structure (columns and types)
...
def _(con, display_result, plot_bar):
    ...
    plot_bar(_df, "name", "age",
             title="Students Aged 20–21",
             xlabel="Student", ylabel="Age")
    return
...
def _(con, display_result):
    ...
    df_young = con.execute(sql_youngest).df()
    df_old = con.execute(sql_oldest).df()
...
def _(con, display_result, plot_bar):
    ...
    plot_bar(_df, "grade", "student_count",
...
def _(con, display_result, plot_pie):
    ...
    plot_pie(_df, "country", "student_count",
...
def _(con, display_result, plot_bar):
    ...
    plot_bar(_df, "country", "avg_age",
             title="Average Student Age by Country",
             xlabel="Country", ylabel="Average Age")
    return
...
def _(con, display_result, plot_hbar):
    ...
    plot_hbar(_df, "country", "student_count",
              title="Countries with Multiple Students",
              xlabel="Student Count", ylabel="Country")
    return
...
def _(con, display_result):
    ...
    _sql = """
    INSERT INTO students (id, name, age, grade, country)
    VALUES 
    (11, 'Kate', 21, 'B', 'USA');
    """
    ...
    #----------------------
    _df = con.execute("SELECT * FROM students WHERE id = 11;").df()
...
def _(con, display_result):
    ...
    df_before = con.execute("SELECT * FROM students WHERE name = 'Bob';").df()
    display_result(df_before, "BEFORE Update")
    ...
    df_after = con.execute("SELECT * FROM students WHERE name = 'Bob';").df()
    display_result(df_after, "AFTER Update (grade changed to A)")
    return
...
def _(con, display_result):
    ...
    df_target = con.execute("SELECT * FROM students WHERE id = 10;").df()
    display_result(df_target, "⚠️ Record to be DELETED")
    ...
    # Verify deletion
    _count = con.execute("SELECT COUNT(*) FROM students;").fetchone()[0]
    print(f"\n✅ Deleted. Students remaining: {_count}")
    return
...
def _(con, display_result, plot_bar):
    ...
    plot_bar(_df, "name", "age",
             title="All Students — Final Dataset",
             xlabel="Student", ylabel="Age", rotate_x=30)
    return
...
def _(mo):
    ...
    |---------|---------|---------|
    | `CREATE TABLE` | Define table structure | `CREATE TABLE students (id INT PRIMARY KEY, ...)` |
