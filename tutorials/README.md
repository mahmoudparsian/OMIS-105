...
| [`Introduction_to_RDBMS_with_DuckDB/`](Introduction_to_RDBMS_with_DuckDB/) | 12-part, ~2-hour self-paced course covering the whole syllabus in one sequence: relational model & keys, `CREATE TABLE`/`INSERT`, `SELECT`/`WHERE`/`ORDER BY`, joins, aggregation, subqueries/CTEs, constraints & ACID, normalization, views & transactions, DuckDB superpowers, and a capstone | Weeks 1–9 — comprehensive; see the folder's own README for notebook-by-notebook mapping |
