...
For this course, GitHub is where all lectures, notebooks, labs, and data
files are published. When the instructor adds new material, it appears
there first.
...
same commands as a Mac terminal. Use Git Bash for everything in this
guide.
...
That creates a folder named `OMIS-105` on your Desktop. Go into it and
look around:
...
**The safe habit:** treat your `OMIS-105` folder as read-only. When you
want to work on a lab or notebook, **copy it** to your own work folder
first:

```bash
mkdir -p ~/Desktop/my_omis105_work
cp ~/Desktop/OMIS-105/tutorials/GROUP_BY/*.py ~/Desktop/my_omis105_work/
```
...
If you are stuck, the simplest fix always works: delete the `OMIS-105`
folder and clone it again (Section 4).
...
* A **commit message** should say *what* you changed:
  `"add week 3 lab answers"`, not `"stuff"`.
* `main` is the name of the default branch — the main line of history.
...
habit for your career.

Start tracking a folder of your own:

```bash
cd ~/Desktop/my_omis105_work
git init
git add .
...
| `fatal: not a git repository` | You are in the wrong folder — `cd` into `OMIS-105` first |
